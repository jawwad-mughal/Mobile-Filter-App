...
var mobiles = {
  ...
  function createMobileModelDetailList() { 
    let value = mobileDetails

    let arrayLenght = value.length

    if (arrayLenght === 8) {
      
      let processor = `Processor: ${value[0]}` 
      
      let memory = Object.values(value[1])
      let memoryValues =`Memory: Ram: ${memory[0]}, Storage: ${memory[1]}` 
      
      
      let brandName =`Brand Name: ${value[2]}` 
      
  
      let camera = Object.values(value[3])
      
      let cameraArray1 = Object.values(camera[0]) 
  
      let rear = `Rear: Main: ${cameraArray1[0]}, Ultrawide: ${cameraArray1[1]}, Telephoto: ${cameraArray1[2]}, TelephotoPeriscope: ${cameraArray1[3]}`
      
  
      let fontValue = `Font: ${camera[1]}`
      
      
      let batteryCapacity = `Battery Capacity: ${value[4]}`
      
      
      let operatingSystem = `Operating System: ${value[5]}`
      
  
      let specialFeatures = `Special Features: "${value[6][0]}" , "${value[6][1]}"`
      

      let image = value[7]
      

      mobileCard(image,processor,memoryValues,brandName,rear,fontValue,batteryCapacity,operatingSystem,specialFeatures)
    }

    if (arrayLenght === 10 && currentBrand === "xiomi") {
      
      let modelName = `Model Name: ${value[0]}` 
      
      let display = Object.values(value[1])
      let displayValues =`Display: size: ${display[0]}, resolution: ${display[1]}, panelType: ${display[2]}, refreshRate: ${display[3]}` 
      
      
      let processor =`Processor: ${value[2]}` 
      
  
      let memory = Object.values(value[3])
      let memoryValues = `Memory: Ram: ${memory[0]}, Storage: ${memory[1]}`
      
      
      let brandName =`Brand Name: ${value[4]}`  
      
      
      let camera = Object.values(value[5])
      let cameraArray1 = Object.values(camera[0])
      let rear = `Rear: Main: ${cameraArray1[0]}, Ultrawide: ${cameraArray1[1]}, Telephoto: ${cameraArray1[2]}`
      
  
      let fontValue = `Font: ${camera[1]}`
      
      
      let batteryCapacity = `Battery Capacity: ${value[6]}`
      
      
      let operatingSystem = `Operating System: ${value[7]}`
      
  
      let specialFeatures = `Special Features: "${value[8][0]}" , "${value[8][1]}"`
      

      let image = value[9]
      

      mobileCard(image,modelName,displayValues,processor,memoryValues,brandName,rear,fontValue,batteryCapacity,operatingSystem,specialFeatures)

    }

    if (arrayLenght === 10 && currentBrand === "realme") {
      
      let modelName = `Model Name: ${value[0]}` 
      
      let display = Object.values(value[1])
      let displayValues =`Display: size: ${display[0]}, resolution: ${display[1]}, Type: ${display[2]}` 
      ...
      let camera = Object.values(value[5])
      let cameraArray1 = Object.values(camera[0])
      let rear = `Rear: Main: ${cameraArray1[0]}`
      
      
      let font = Object.values(camera[1])
      let fontValue = `Font: ${font}`
      
      
      let battery = Object.values(value[6])
      let batteryCapacity = `Battery: Capacity: ${battery[0]}, Fast Charging: ${battery[1]}`
    ...
    if (arrayLenght === 16) {
      ...
      let displayValues =`Display: size: ${value[1]}` 
      
      
      let displayResolution = `displayResolution: ${value[2]}`
     

      let displayType = `displayType: ${value[3]}`
      
      let processor =`Processor: ${value[4]}` 
      
  
      let memory = Object.values(value[5])
      let memoryValues = `Memory: Ram: ${memory[0]}, Storage: ${memory[1]}`
      
      
      let brandName =`Brand Name: ${value[6]}`  
      
      
      let camera = Object.values(value[7])
      
    
      let cameraArray1 = `megapixel: ${camera[0]}, features: ${camera[1]}`
      

      let frontCamera = Object.values(value[8]) 
      
      let frontCameraValue =`FrontCamera: ${frontCamera[0]}pixel` 
      
     
      let batteryCapacity = `Battery: Capacity: ${value[9]}, Fast Charging: ${value[10]}`
      
      
      let operatingSystem = `Operating System: ${value[11]}`
      
  
      let specialFeatures = `fingerprintSensor: ${value[12]}`
      

      let waterResistance = `waterResistance: ${value[13]}`
      ...
      // create card function call
      mobileCard(image,modelName,displayValues,processor,memoryValues,brandName,cameraArray1,frontCameraValue,batteryCapacity,operatingSystem,specialFeatures,waterResistance,expandableStorage)
  ...
  // Create mobile Details card

  function mobileCard(imageSrc,...cardValue) {
    ...
    let para = document.querySelectorAll('#description>p')
    // console.log(para)
